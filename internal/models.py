...
class Model(nn.Module):
    ...
    # How much to dilate intervals relatively.
    ...
    # Upper bound on max number of train images.
    num_glo_embeddings: int = 1000
    # Learned exposure scaling (RawNeRF).
    learned_exposure_scaling: bool = False
    # How fast to anneal in near bound.
    near_anneal_rate: Optional[float] = None
    # Where to initialize near bound (in [0, 1]).
    ...
    def __call__(
        ...
        if self.num_glo_features > 0:
            if not zero_glo:
                ...
                glo_vecs = nn.Embed(self.num_glo_embeddings,
                                    self.num_glo_features)
        ...
        for i_level in range(self.num_levels):
            ...
            if i_level >= 1 and self.rvs:
                ...
                #if rng is not None: 
                #    key, rng = random_split(rng)
                #    sdist = sdist + 0.001 * jax.random.uniform(key, shape=sdist.shape)
                #    sdist = jnp.clip(sdist, init_s_near, init_s_far - 0.0001)
                #    sdist = jnp.sort(sdist, axis=-1) 
                tdist = s_to_t(sdist)
            ...
            if rays.exposure_idx is not None:
                ...
                if self.learned_exposure_scaling:
                    ...
                    scaling = 1 + mask[..., None] * \
                        exposure_scaling_offsets(exposure_idx)
